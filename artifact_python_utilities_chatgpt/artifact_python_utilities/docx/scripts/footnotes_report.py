...
def _report_part(z: zipfile.ZipFile, part: str, kind: str) -> None:
    ...
    print(f"[{part}] defined_{kind}s={len(ids)} ids={sorted(ids)[:10]}{'...' if len(ids)>10 else ''}")
    for n in notes[:5]:
        nid = n.get(f"{{{W_NS}}}id")
        txt = _note_text(n)
        if txt:
...
def main() -> None:
    ...
    with zipfile.ZipFile(args.docx, "r") as z:
        ...
        fn_refs = [int(x.get(f"{{{W_NS}}}id")) for x in doc.xpath(".//w:footnoteReference", namespaces=NS)]
        en_refs = [int(x.get(f"{{{W_NS}}}id")) for x in doc.xpath(".//w:endnoteReference", namespaces=NS)]
        ...
        if fn_refs:
            ...
            if dupes:
                print(f"[warn] duplicated footnote reference ids (multiple references to same note): {dupes}")
