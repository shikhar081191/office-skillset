...
def main() -> None:
    ...
    if args.cmd == "list":
        raise SystemExit(
            cmd_list(args.docx, include_headers_footers=not args.no_headers_footers, as_json=args.json)
        )
    if args.cmd == "wrap_placeholders":
        raise SystemExit(
            cmd_wrap(args.docx, args.output, include_headers_footers=not args.no_headers_footers)
        )
    if args.cmd == "fill":
        if not args.set:
            raise SystemExit("fill requires at least one --set TAG=VALUE")
        raise SystemExit(
            cmd_fill(args.docx, args.output, include_headers_footers=not args.no_headers_footers, pairs=args.set)
        )
