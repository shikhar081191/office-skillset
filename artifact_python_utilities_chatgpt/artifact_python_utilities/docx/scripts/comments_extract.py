...
def _load_comments(z: zipfile.ZipFile) -> dict[str, dict[str, Any]]:
    ...
    for c in root.xpath(".//w:comment", namespaces=NS):
        ...
        out[cid] = {
            ...
            "resolved": True if done == "1" else False,
...
def _collect_ranges(root: etree._Element) -> dict[str, dict[str, Any]]:
    ...
    for p in root.xpath(".//w:p", namespaces=NS):
        ...
        for s in starts:
            cid = s.get(f"{{{W_NS}}}id")
            if cid:
                ranges.setdefault(cid, {}).setdefault("paragraphs", []).append({"where": "start", "text": ptxt[:200]})
        for e in ends:
            cid = e.get(f"{{{W_NS}}}id")
            if cid:
                ranges.setdefault(cid, {}).setdefault("paragraphs", []).append({"where": "end", "text": ptxt[:200]})
    return ranges
