...
def main() -> None:
    ...
    ap.add_argument("--mode", required=True, help="Protection mode: off | readOnly | comments | trackedChanges | forms (aliases accepted: read_only, read-only, readonly, tracked_changes, tracked-changes)")
    ...
    with zipfile.ZipFile(args.in_docx, "r") as zin:
        ...
        if key not in mode_map:
            raise SystemExit(f"[set_protection] invalid --mode={mode_in!r}. Use off, readOnly, comments, trackedChanges, forms (aliases: read_only, tracked_changes).")
