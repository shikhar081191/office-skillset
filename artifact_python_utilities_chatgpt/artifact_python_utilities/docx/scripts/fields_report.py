...
def _iter_word_xml_parts(z: zipfile.ZipFile):
    ...
    for name in z.namelist():
        ...
        if base in {"document.xml", "settings.xml"}:
            yield name
        elif base.startswith("header") or base.startswith("footer"):
            yield name
...
def main() -> None:
    ...
    if needs_update:
        print("\nREMINDER")
        print("- If the PDF/PNGs show placeholders or wrong page numbers/TOC, open in Word and run: Ctrl+A → F9 (Update Fields), then re-render.")
